...
def train(train_set,
          ...
          log,
          batch_size=None,
          batches_between_logging=None,
          evaluate_after_batches_between_logging=None,
          dropout=None,
          n_crit=None,
          lambd=None,
    ...
    n_critic, lambda, dropout, shared_hidden_size, F_layers, C_layers, D_layers: hyperparameters that can be set
    If hyperparameters are not set, values are taken from opt
    since we have all information in one dataset we don't need to loop over different datasets, domains etc...
    """
    # initialize these parameters or take their value from params
    batch_size = batch_size or params.training_setting["batch_size"]
    batches_between_logging = batches_between_logging or params.training_setting["batches_between_logging"]
    evaluate_after_batches_between_logging = \
        evaluate_after_batches_between_logging or params.training_setting["evaluate_after_batches_between_logging"]
    dropout = dropout or params.model_params["dropout"]
    lambd = lambd or params.model_params["lambd"]
    n_crit = n_crit or params.model_params["n_critic"]
    shared_hidden_size = shared_hidden_size or params.model_params["shared_hidden_size"]
    D_learning_rate = D_learning_rate or params.training_setting["D_learning_rate"]
    learning_rate = learning_rate or params.training_setting["learning_rate"]
    C_layers = C_layers or params.model_params["C_layers"]
    D_layers = D_layers or params.model_params["D_layers"]
    F_layers = F_layers or params.model_params["F_layers"]

    hyperopt_string = f"bs{batch_size}dr{dropout}lambd{lambd}ncrit{n_crit}shs{shared_hidden_size}" \
                      f"Dlr{D_learning_rate}lr{learning_rate}CL{C_layers}DL{D_layers}FL{F_layers}"

    if params.training_setting["use_tensorboard"]:
        tensorboard_dir_path = os.path.join(params.training_setting["tensorboard_dir"], hyperopt_string)
        from torch.utils.tensorboard import SummaryWriter
        writer = SummaryWriter(log_dir=tensorboard_dir_path, comment='Using lambda {}'.format(
            params.model_params["lambd"]))
        print(f"tensorboard logging path is {tensorboard_dir_path}")

    # dataset loaders, make usual PyTroch Loader
    ...
    F_s = MlpFeatureExtractor(params.model_params["feature_num"], params.model_params["F_hidden_sizes"],
                              shared_hidden_size, dropout, params.model_params["F_bn"])
    C = SentimentClassifier(C_layers, shared_hidden_size + params.model_params["domain_hidden_size"],
                            shared_hidden_size + params.model_params["domain_hidden_size"],
                            params.model_params["num_labels"], dropout, params.model_params["C_bn"])
    D = DomainClassifier(D_layers, shared_hidden_size, shared_hidden_size,
                         params.model_params["all_domains"], params.model_params["loss"],
                         dropout, params.model_params["D_bn"])
    ...
    optimizer = optim.Adam(itertools.chain(*map(list, [F_s.parameters() if F_s else [], C.parameters()])),
                           lr=learning_rate)
    optimizerD = optim.Adam(D.parameters(), lr=D_learning_rate)
    ...
    if params.training_setting["test_only"]:
        ...
        log.info(f'Average validation accuracy: {dev_classification_report_dict["accuracy"]}')
        ...
        with open("../../KnowMan/save/labels_best_spouse_tfidf.csv", "w")as out:
    ...
    # training
        ...
        for epoch in range(params.training_setting["max_epoch"]):
            ...
            for critic_loop_index in range(n_crit):
                ...
                for i, (inputs, labels, adv_labels) in enumerate(train_loader):
                    ...
                    if (i > 0 and i % batches_between_logging == 0) or i == num_training_items - 1:
                        ...
                        writer.add_scalar("Domain Training Accuracy", d_correct / d_total,
                                          total_steps)
                    ...
                    if evaluate_after_batches_between_logging or i == num_training_items - 1:
                        if tmp_dev_classification_report_dict['1']["f1-score"] > best_weighted_f1:
                            ...
                            torch.save(F_s.state_dict(),
                                       '{}/netF_s.pth'.format(params.training_setting["model_save_file"]))
...
def main():
    ...
    spouse_params.update_parameters("./spouse_tfidf.yaml")
    ...
    train_dataset, dev_dataset, test_dataset = get_dataset(spouse_params.dataset["dataset_path"])
    cv = train(train_dataset, dev_dataset, test_dataset, spouse_params, log)
