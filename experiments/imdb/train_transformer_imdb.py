...
def train(train_set, dev_set, test_set, params, log, feature_ext_enum=FeatureExtraction.DISTILBERT):
    ...
    if params.training_setting["test_only"]:
        ...
        with open("../../KnowMan/save/labels_best_imdb_bert.csv", "w")as out:
            for i in range(len(y_true)):
                out.write(str(y_true[i]) + "," + str(y_pred[i]) + "\n")

        print({'valid': dev_classification_report_dict["accuracy"],
               'test': test_classification_report_dict["accuracy"]})
    ...
    # training
        ...
        for epoch in range(params.training_setting["max_epoch"]):
            ...
            for i, batch_X in enumerate(train_loader):
                ...
                if (i > 0 and i % batches_between_logging == 0) or i == num_training_items-1:
                    ...
                    if evaluate_after_batches_between_logging or i == num_training_items - 1:
                        if tmp_dev_classification_report_dict['accuracy'] > best_acc:
                            log.info(f'New best average validation accuracy: '
                                     f'{100.0 * tmp_dev_classification_report_dict["accuracy"]}%')
                            best_avg_acc['valid'] = tmp_dev_classification_report_dict['accuracy']
                            best_avg_acc['test'] = tmp_test_classification_report_dict['accuracy']
                            best_acc = tmp_dev_classification_report_dict['accuracy']
        ...
        # end of training
        log.info(f'Best average validation accuracy: {100.0*best_acc}%')
        return best_avg_acc
...
def main():
    imdb_params = KnowManParameters()
    imdb_params.update_parameters("./imdb_transformer.yaml")

    set_seeds(imdb_params)
    log = set_logging(imdb_params)

    if not os.path.exists(imdb_params.training_setting["model_save_file"]):
        os.makedirs(imdb_params.training_setting["model_save_file"])

    feature_ext_enum = FeatureExtraction.name2type(imdb_params.model_params["feature_ext"])

    train_dataset, dev_dataset, test_dataset = get_transformer_dataset(imdb_params.dataset["dataset_path"],
                                                                       feature_ext=feature_ext_enum,
                                                                       max_length_transformer=
                                                                       imdb_params.model_params
                                                                       ["max_length_transformer"])

    cv = train(train_dataset, dev_dataset, test_dataset, imdb_params, log, feature_ext_enum=feature_ext_enum)
