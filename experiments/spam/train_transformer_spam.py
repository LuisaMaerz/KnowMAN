...
def train(train_set, test_set, params, log, feature_ext_enum=FeatureExtraction.DISTILBERT):
    """
    ...
    if params.training_setting["test_only"]:
        ...
        with open("../../KnowMan/save/labels_l0_spam_bert.csv", "w")as out:
            ...
            for critic_loop_index in range(params.model_params["n_critic"]):
                ...
                for i, batch_X in enumerate(train_loader):
                    ...
                    if (i > 0 and i % batches_between_logging == 0) or i == num_training_items - 1:
                        ...
                        discr_loss_collection = []
                        log.info("Discriminator training Epoch {}, critic_loop_index {}, Step {}...".format(
                            epoch, critic_loop_index, i))
                        ...
                        if d_total > 0:
                            log.info('Domain Training Accuracy: {}%'.format(d_correct / d_total))
...
def main():
    ...
    spam_params.update_parameters("./spam_transformer.yaml")
    ...
    feature_ext_enum = FeatureExtraction.name2type(spam_params.model_params["feature_ext"])
    train_dataset, test_dataset = get_transformer_dataset(spam_params.dataset["dataset_path"],
                                                          feature_ext=feature_ext_enum, if_dev_data=False,
                                                          max_length_transformer=
                                                          spam_params.model_params["max_length_transformer"])

    cv = train(train_dataset, test_dataset, spam_params, log, feature_ext_enum=feature_ext_enum)
